...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['file']:
            ...
            # Default: look for the CSV in the Django project root
            base_dir = os.path.dirname(os.path.dirname(os.path.dirname(
                os.path.dirname(os.path.abspath(__file__))
            )))
            csv_path = os.path.join(base_dir, 'C://smart_diet_app//mynaijadiet_dataset_v2.csv')
        ...
        with open(csv_path, newline='', encoding='utf-8') as csvfile:
            ...
            for row_num, row in enumerate(reader, start=2):  # start=2 (row 1 = header)
                ...
                # Skip duplicates (update instead of error)
